...
- **Fiche personnage : le squelette suit le sujet.** `characterSheetFromBrief` fait d'abord trancher gemma sur `kind` (`human` / `other`) ; ce champ choisit la liste de champs (`CHAR_FIELD_KEYS` / `CHAR_OTHER_FIELD_KEYS`), les étiquettes du menu Personnage et le gabarit de planche (`CHARSHEET_HUMAN` / `CHARSHEET_SUBJECT`). Un gabarit humain en dur (« one single person », « standing poses », « costume details ») rendait un lion anthropomorphe habillé sur un brief animalier, et faisait diverger les keyframes plan par plan. Le gabarit non humain est **générique** (morphologie entièrement déléguée à `charDesc`) pour couvrir animal, créature, robot non humanoïde, extraterrestre ou objet sans un gabarit par famille — ne pas le spécialiser. Détail : LESSONS piège n°17.
