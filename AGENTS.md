...
- Enrichissement de prompt : **exclusivement `gemma4:e4b`** via Ollama (ne pas exposer d'autres LLM locaux). Deux consignes distinctes selon le moteur ciblé : `ENRICH_SYSTEM` (générique) et `KREA2_ENRICH_SYSTEM` (ordre sujet→style→lumière→optique→composition→couleur→matière→atmosphère, 40–100 mots, remplissage banni, **aucun négatif demandé**) — sélection par `isKrea2Workflow()`. gemma n'honore pas de façon fiable l'interdiction de vocabulaire : `stripPromptPadding()` retire le remplissage de sa sortie après coup (chemin Krea 2 uniquement), ne jamais compter sur la seule consigne. Un workflow marqué `"enrich": "builtin"` dans le manifest n'est jamais enrichi génériquement : c'est le cas de LTX 2.5 (enhancer en graphe) et de `storyboard_v2`/`minimax_h3_r2v`, qui construisent eux-mêmes leurs prompts à partir du brief brut.
