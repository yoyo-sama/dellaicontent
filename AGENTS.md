...
Démo Dell GB10 : page statique unique (`index.html`) qui pilote ComfyUI (`:8188`) et Ollama (`:11434`). Pas de build, pas de dépendances côté frontend. Publique via nginx sur `:8090` (`docker compose up -d`). Trois stacks Docker distinctes, une par service, à la racine du home : `~/ai-content-studio` (app + updater), `~/comfyui-spark` (ComfyUI, gabarit `docker/stacks/comfyui.yml`), `~/ollama` (Ollama, gabarit `docker/stacks/ollama.yml`) — `install.sh` détecte l'état réel de la machine (empreinte, jamais le numéro `VERSION`) et propose un plan en trois modes (fresh/repair/uninstall) confirmé avant toute action ; modèles et données ne sont jamais supprimés, un ComfyUI tiers déjà en place est réutilisé tel quel, ou une reprise en main lui est proposée s'il est mal configuré pour l'app. Harnais : `bash tests/selftest-install.sh`. Seule exception backend : le service `updater` (`docker/updater/`, Python stdlib, `127.0.0.1:8093`) qui expose `/update/status`, `/update/apply` et `/update/loras/upload` (proxifiés par nginx) pour la mise à jour git déclenchée depuis l'UI et le téléversement de LoRA — voir le changelog du 2026-09-07 (suite 3) et du 2026-09-08 (suite).
...
`TOUR-DE-CONTROLE-CHANGELOG.md` (racine) journalise les orchestrations passées (lots, décisions, fichiers touchés) ; `docs/NOUVEAUX-MODELES-LOT1.md` détaille la qualification des modèles Krea 2/LTX 2.5/Minimax H3. Ce sont des comptes-rendus historiques (à lire pour le contexte), pas une doc d'état courant — `docs/ARCHITECTURE.md`, `docs/INSTALL.md` (anglais, guide complet d'`install.sh`), `docs/LESSONS.md`, `docs/TESTING.md`, `docs/TROUBLESHOOTING.md` (anglais) et `docs/TROUBLESHOOTING.fr.md` (français) — dépannage installation/déploiement : erreurs `JSON.parse`/`NetworkError`, modèles invisibles, Ollama natif, remise à zéro), ce fichier et les README sont les sources à jour.
