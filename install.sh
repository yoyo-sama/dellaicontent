#!/usr/bin/env bash
# install.sh — the Dell AI Content Studio installer for GB10/DGX Spark.
#
# Same job as the previous installer (v1) (deploy nginx web + updater here, ComfyUI in
# ~/comfyui-spark, Ollama in ~/ollama), rebuilt around the lessons that v1 learned the hard way:
#
...
#
# Usage:
#   ./install.sh                 install / repair / update, idempotent
#   ./install.sh --check         diagnose only, change nothing (the runbook, as code)
#   ./install.sh --dry-run       print the plan and each step it would take, change nothing
#                                 (with --mode uninstall: what it would remove)
#   ./install.sh --mode M        fresh | repair: only the plan's header changes — the steps always
#                                 come from the state of the machine. uninstall: remove the
#                                 containers this installer created, never any data
#   ./install.sh --skip-models   download no model file. The Ollama model is still pulled, legacy
#                                 models are still moved, and missing model files still exit 2
#   ./install.sh --yes           answer yes to the plan confirmation, to the takeover of a broken
...
#   ./install.sh --components L  uninstall only: comma-separated web,updater,comfyui,ollama
#   ./install.sh --smoke         after the install, prove it with a reduced real render (python3)
#   ./install.sh --log FILE      log file (default ~/install-YYYY-MM-DD-HHMM.log)
#
...
SMOKE=0
COMPONENTS=""
LOG_FILE="$HOME/install-$(date +%F-%H%M).log"
...
# ── Asking the user ───────────────────────────────────────────────────────────
# INSTALL_ASSUME_TTY is a documented test hook: the harness pipes the answers in on stdin,
...
interactive() { [ -t 0 ] || [ -n "${INSTALL_ASSUME_TTY:-}" ]; }
...
fingerprint() {
  ...
  # WANTED_UID: "${WANTED_UID:-1000}" (a variable) and install.sh writes the .env next to it;
...
TAKEOVER_MARK="# written by install.sh — remove this file to restore the original definition"
...
takeover_blocker() {
  ...
  elif [ -f "$FP_COMFY_OVR" ] && ! head -n1 "$FP_COMFY_OVR" | grep -qE 'written by install\.sh|written by install2\.sh'; then
