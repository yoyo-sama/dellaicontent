...
- **Fiche personnage : le squelette suit le sujet.** `characterSheetFromBrief` fait d'abord trancher gemma sur `kind` (`human` / `animal`) ; ce champ choisit la liste de champs (`CHAR_FIELD_KEYS` / `CHAR_ANIMAL_FIELD_KEYS`), les étiquettes du menu Personnage et le gabarit de planche (`CHARSHEET_HUMAN` / `CHARSHEET_ANIMAL`). Un gabarit humain en dur (« one single person », « standing poses », « costume details ») rendait un lion anthropomorphe habillé sur un brief animalier, et faisait diverger les keyframes plan par plan. Détail : LESSONS piège n°17.
